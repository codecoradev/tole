...
| Excessive agency / tool misuse | **gap → fixed here (RC-1)** + documented | `run_command`/`job_start` were plain `Risk::Write` while capable of catastrophic destruction; catastrophic argv patterns now refuse with a Destructive-tier pointer. Full per-call risk classification = tracked follow-up. |
